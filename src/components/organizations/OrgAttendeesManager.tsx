...
export default function OrgAttendeesManager({ orgId, orgName }: OrgAttendeesManagerProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [attendeesResponse, statsResponse, formResponse] = await Promise.all([
      ...
      {attendees.length === 0 ? (
        <Alert variant="light" color="blue">
          ...
          <Text size="sm" c="dimmed" mt="xs">
            Los registros se crearán automáticamente cuando los usuarios se registren en eventos,
            o puedes agregarlos manualmente.
          </Text>
        </Alert>
      ) : (
                ...
                <Table.Tr key={attendee._id}>
                  {getIdentifierFields().map(field => (
                    <Table.Td key={field.id}>
                      <Text size="sm">
                        {getFieldValue(attendee, field.id)}
                      </Text>
                    </Table.Td>
                  ))}
                  <Table.Td>
                    <Badge variant="light" size="sm">
                      {attendee.eventIds.length} evento{attendee.eventIds.length !== 1 ? 's' : ''}
                    </Badge>
                  </Table.Td>
                  <Table.Td>
              ...
              <Card withBorder p="sm">
                <Stack gap="sm">
                  {selectedAttendee.registrationData && Object.keys(selectedAttendee.registrationData).length > 0 ? (
                    <>
                      {Object.entries(selectedAttendee.registrationData).map(([fieldId, value]) => {
                      ...
                      <Group justify="space-between">
                        <Text size="sm" c="dimmed">Eventos registrados:</Text>
                        <Badge variant="light" size="sm">
