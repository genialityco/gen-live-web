...
export type { RegistrationForm, FormField, FormFieldOption, FormFieldType } from '../types';
...
export async function createOrg(payload: { name: string; domainSlug: string; branding?: any }) {
...
export async function updateOrgBranding(slug: string, branding: Partial<BrandingConfig>) {
  const { data } = await api.patch(`/orgs/slug/${slug}/branding`, branding);
  return data as Org;
}

export async function uploadBrandingImage(slug: string, folder: string, file: File) {
  const formData = new FormData();
  formData.append('file', file);
  
  const { data } = await api.post(`/orgs/slug/${slug}/upload/${folder}`, formData, {
    headers: {
...
export async function updateRegistrationForm(slug: string, form: RegistrationForm) {
  const { data } = await api.patch(`/orgs/slug/${slug}/registration-form`, form);
  return data as Org;
}
...
export async function updateOrganization(slug: string, payload: { name?: string; description?: string }) {
